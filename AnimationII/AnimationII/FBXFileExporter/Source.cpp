			...
			*numVertices = mesh->GetControlPointsCount();
			...
			for (int j = 0; j < *numVertices; j++)
			{
				...
				for (int k = 0; k < MAX_INFLUENCES; ++k)
				{
					...
					vertices->data()[j].weights[k] = _theSet[k].weight;
				}
			}
...
#pragma region Animation
	...
	for (int i = 0; i < _clusterCount; ++i)
	{
		...
		//if (JOINT_INDEX != -1)
		//{
		//	int controlPointIndexCount = _currClust->GetControlPointIndicesCount();

		//	double* weights = _currClust->GetControlPointWeights();
		//	int* indices = _currClust->GetControlPointIndices();

		//	for (int controlPointIndex = 0; controlPointIndex < controlPointIndexCount; controlPointIndex++)
		//	{
		//		influence toAdd;

		//		toAdd.joint = JOINT_INDEX;
		//		toAdd.weight = weights[controlPointIndex];

		//		int minIndex = 0;
		//		for (int i = 1; i < 4; i++)
		//		{
		//			if (control_point_influences[indices[controlPointIndex]][i].weight <
		//				control_point_influences[indices[controlPointIndex]][minIndex].weight)
		//			{
		//				minIndex = i;
		//			}
		//		}

		//		if (toAdd.weight > control_point_influences[indices[controlPointIndex]][minIndex].weight)
		//		{
		//			control_point_influences[indices[controlPointIndex]][minIndex] = toAdd;
		//		}
		//	}
		...
		for (int j = 0; j < _currClust->GetControlPointIndicesCount(); ++j)
		{
			...
			double WEIGHT = _currClust->GetControlPointWeights()[j];
	...
	//{
	//	float sum = control_point_influences[i][0].weight +
	//		control_point_influences[i][1].weight +
	//		control_point_influences[i][2].weight +
	//		control_point_influences[i][3].weight;

	//	control_point_influences[i][0].weight /= sum;
	//	control_point_influences[i][1].weight /= sum;
	//	control_point_influences[i][2].weight /= sum;
	//	control_point_influences[i][3].weight /= sum;
	//}
