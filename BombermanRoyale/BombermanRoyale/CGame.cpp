...
static std::vector<key> keys(20);

struct KEYS
{
	enum { UP, DOWN, LEFT, RIGHT, ZERO, RMB, SPACE, P2UP, P2LEFT, P2DOWN, P2RIGHT, P1UP, P1LEFT, P1DOWN, P1RIGHT, P2BOMB, P1BOMB, HELP_MENU, GAME_STATE, FULLSCREEN };
};
...
#pragma endregion
	...
	while (G_SUCCESS(g_pWindow->GetLastEvent(gLastEvent)) && gLastEvent != GW::SYSTEM::GWindowInputEvents::DESTROY)
	{
		...
		static int prevNumPlayersAlive = numPlayersAlive;
...
void CGame::GamePlayLoop(double timePassed)
{
	...
	for (CPlayer* currPlayer : v_cPlayers)
	{
		if (!currPlayer || !currPlayer->isAlive())
			continue;
		float isUpPressed, isDownPressed, isLeftPressed, isRightPressed, isSouthPressed;
			...
			for (int i = 0; i < objects.size() - 2; ++i)
			{
				if (p_cRendererManager->HasComponent(*(objects[i]), COMPONENT_TYPE::RENDERER))
				{
					...
					if (objects[i]->GetComponent(COMPONENT_TYPE::TRANSFORM, cTransform))
					{
						...
						if (dZ < 2.0f && !transform->nFloor && dX < 2.05f)
						{
			...
			for (int i = 0; i < objects.size() - 2; ++i)
			{
				if (p_cRendererManager->HasComponent(*(objects[i]), COMPONENT_TYPE::RENDERER))
				{
					...
					if (objects[i]->GetComponent(COMPONENT_TYPE::TRANSFORM, cTransform))
					{
						...
						float dZ = abs(pTransform->fPosition.z - transform->fPosition.z);
						float dX = abs(pTransform->fPosition.x - transform->fPosition.x);
						if (dZ < 1.6f && !transform->nFloor && dX < 2.05f)
						{
							if (pTransform->fPosition.z > transform->fPosition.z)
							{
								canMove = false;
		...
		for (int i = 0; i < Xexplosions.size(); ++i)
		{
			if (!currPlayer->isAlive())
				continue;
			DirectX::XMFLOAT3 pPos, xPos;
		...
		for (int i = 0; i < Zexplosions.size(); ++i)
		{
			if (!currPlayer->isAlive())
				continue;
			DirectX::XMFLOAT3 pPos, zPos;
			pPos = pTransform->fPosition;
			TComponent* zT = nullptr;
			if (Zexplosions[i]->GetComponent(COMPONENT_TYPE::TRANSFORM, zT))
			{
				TTransformComponent* zTrans = (TTransformComponent*)zT;
...
void CGame::updateBombs(double timePassed)
{
	...
	for (int i = 0; i < v_cBombs.size(); ++i)
	{
		...
		if (v_cBombs[i]->isAlive() && v_cBombs[i]->shouldExplode())
		{
			v_cBombs[i]->Explode();
