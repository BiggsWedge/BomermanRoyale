...
#pragma endregion
	...
	while (G_SUCCESS(g_pWindow->GetLastEvent(gLastEvent)) && gLastEvent != GW::SYSTEM::GWindowInputEvents::DESTROY) {
		...
		//render particles
		//InitSortedParticles(sortedParticles, timer.Delta());
...
void CGame::GamePlayLoop(double timePassed)
{
	...
	for (CPlayer* currAI : v_cAI)
	{
		...
		if (fMaxX == 15.0f)
		{
			...
			if (dec > 0.0f)
				x = abs((fMaxX - 2.5f) - AITransform->fPosition.x + 1.25f) / 2.5f;
...
void CGame::InitSortedParticles(end::sorted_pool_t<particle, 1000>& sortedPool, float deltaTime) {
...
void CGame::InitFreeParticles(emitter& emitter, end::pool_t<particle, 1024>& freePool, float deltaTime) {
	//init emitters
